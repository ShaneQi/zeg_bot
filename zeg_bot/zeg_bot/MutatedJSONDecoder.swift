...
/// - `JSONArray`
/// - `JSONDictionary`
public class MutatedJSONDecoder {
		...
		while let c = next {
			...
			if esc {
				...
				case "u":
					...
					var uintValue = UInt32(strtoul(hexStr, nil, 16))
					
					if case 55296...56319 = uintValue {
						var nextHexStr = ""
						self.next()
						self.next()
						for _ in 1...4 {
							...
							nextHexStr.append(hexC)
						}
						let nextUintValue = UInt32(strtoul(nextHexStr, nil, 16))
						
						uintValue = (uintValue - 55296) * 1024 + (nextUintValue - 56320) + 65536
					}
					
					let result = UnicodeScalar(uintValue)
					s.append(result)
				default:
					s.append(c)
				}
